<script lang="ts">
    ...
    let title = $state('Eureka Bunnies');
    let description = $state('Possibilities for eureka bunny coffer content.');
    ...
    if (cofferSearchParams !== undefined) {
        ...
        if (selection !== undefined) {
            title = `Eureka Bunnies - ${selection.coffer.Name}`;
    ...
    function openTab(territoryId: number, cofferId: number, addQuery: boolean = false) {
        ...
        // Set the new title
        document.title = `Eureka Bunnies - ${selection.coffer.Name}`
    }

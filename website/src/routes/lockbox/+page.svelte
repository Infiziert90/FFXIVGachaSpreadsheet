<script lang="ts">
    ...
    let title = $state('Field Operation Lockboxes');
    let description = $state('Possibilities for all lockboxes obtainable in Eureka, Bozja and Zadnor.');
    ...
    if (cofferSearchParams !== undefined) {
        ...
        if (selection !== undefined) {
            title = `Field Operation Lockboxes - ${selection.coffer.Name}`;
    ...
    function openTab(territoryId: number, cofferId: number, addQuery: boolean = false) {
        ...
        // Set the new title
        document.title = `Field Operation Lockboxes - ${selection.coffer.Name}`
    }

<script lang="ts">
    ...
    let title = $state('Field Operation Container');
    let description = $state('Possibilities for all Logograms and Fragments.');
    ...
    if (cofferSearchParams !== undefined) {
        ...
        if (selection !== undefined) {
            title = `Field Operation Container - ${selection.coffer.Name}`;
    ...
    function openTab(territoryId: number, cofferId: number, addQuery: boolean = false) {
        ...
        // Set the new title
        document.title = `Field Operation Container - ${selection.coffer.Name}`
    }
